...
bool DynamicInitializer::tryDynInit(const std::vector<sensor_msgs::Imu>& imu_msg_buffer,
        CameraMeasurementConstPtr img_msg) {

    // Counter how many IMU msgs in the buffer are used.
    int used_imu_msg_cntr = 0;

    double time_bound = img_msg->header.stamp.toSec()+td;

    for (const auto& imu_msg : imu_msg_buffer) {
        double imu_time = imu_msg.header.stamp.toSec();
        if (imu_time <= lower_time_bound)  continue;
        // threshold is adjusted according to the imu frequency
        if (imu_time-time_bound > imu_img_timeTh) break;  
        ddt = imu_time-time_bound;  
        processIMU(imu_msg);
        ++used_imu_msg_cntr;
    }
...
void DynamicInitializer::processIMU(const sensor_msgs::Imu& imu_msg) {

    Vector3d angular_velocity, linear_acceleration;
    tf::vectorMsgToEigen(imu_msg.angular_velocity, angular_velocity);
    tf::vectorMsgToEigen(imu_msg.linear_acceleration, linear_acceleration);

    // into this if only at the first time
    if (!first_imu) {
        first_imu = true;
        acc_0 = linear_acceleration;
        gyr_0 = angular_velocity;
        curr_time = imu_msg.header.stamp.toSec();
    }

    double dt = imu_msg.header.stamp.toSec() - curr_time;
...
void DynamicInitializer::processImage(CameraMeasurementConstPtr img_msg) {
    ...
    Times[frame_count] = img_msg->header.stamp.toSec();

    ImageFrame imageframe(img_msg, td+ddt);
    imageframe.pre_integration = tmp_pre_integration;
    all_image_frame.insert(make_pair(img_msg->header.stamp.toSec()+td, imageframe));
    tmp_pre_integration.reset(new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count],
                                              acc_n, acc_w, gyr_n, gyr_w});

    if (frame_count == WINDOW_SIZE) {
        bool result = false;
        if((img_msg->header.stamp.toSec()-initial_timestamp) > 0.1) {
            result = initialStructure();    
            initial_timestamp = img_msg->header.stamp.toSec();
        }
        if(result)
            bInit = true;
        else {
...
void DynamicInitializer::assignInitialState(std::vector<sensor_msgs::Imu>& imu_msg_buffer, IMUState& imu_state) {
